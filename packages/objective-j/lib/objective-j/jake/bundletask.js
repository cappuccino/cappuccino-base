...
{
    return {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".gif": "image/gif", ".tif": "image/tiff", ".tiff": "image/tiff"}[(FILE.extension(aFilename)).toLowerCase()];
}
...
var LICENSES_PATH = FILE.join(FILE.absolute(FILE.dirname(module.path)), "LICENSES"),
    LICENSE_PATHS = {"LGPL_v2_1": FILE.join(LICENSES_PATH, "LGPL-v2.1"), "MIT": FILE.join(LICENSES_PATH, "MIT")};
...
{
    return isImage(aResourcePath) && FILE.size(aResourcePath) < 32768 && ("data:" + mimeType(aResourcePath) + ";base64," + base64.encode(FILE.read(aResourcePath, "b"))).length < 32768;
};
BundleTask.prototype.defineResourceTask = function(aResourcePath, aDestinationPath)
{
    ...
    if ((extension !== ".cib" || !FILE.exists(extensionless + ".xib") && !FILE.exists(extensionless + ".nib") || this._preventsNib2Cib) && (extension !== ".xib" && extension !== ".nib" || this.includesNibsAndXibs()))
    {
        filedir(aDestinationPath, [aResourcePath],         function()
        {
            if (FILE.exists(aDestinationPath))
                try {
                    FILE.rmtree(aDestinationPath);
                }
                catch(anException) {
                }
    ...
    (this.environments()).forEach(    function(anEnvironment)
    {
        ...
        filedir(dataURLPath,         function(aTask)
        {
            ...
            prerequisites.forEach(            function(aFilename)
            {
                ...
                var contents = "data:" + mimeType(aFilename) + ";base64," + (FILE.read(aFilename, "b")).decodeToString("UTF-8");
